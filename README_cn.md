...
[![OpenClaw Plugins](https://img.shields.io/badge/OpenClaw%20插件-8-red.svg)](#-openclaw-记忆插件)
...
## 🔔 最近更新
+ 2026-03-22 - 🎉 新增 OpenViking（17K+ stars，字节跳动/火山引擎 AI Agent 上下文数据库）
...
| 插件 | Stars | 简介 | 技术栈 |
|------|-------|------|--------|
| [OpenViking](https://github.com/volcengine/OpenViking) | ![GitHub Repo stars](https://img.shields.io/github/stars/volcengine/OpenViking?style=social) | 字节跳动/火山引擎开源 AI Agent 上下文数据库 — 文件系统范式、L0/L1/L2 三层分级加载、VikingDB 向量搜索、技能自演化 | Python |
...
- **描述：** 字节跳动/火山引擎出品的开源 AI Agent 上下文数据库，专门为 OpenClaw 等 Agent 设计，17K+ GitHub stars。
- **核心特性：**
  - **文件系统范式**：通过统一的文件系统隐喻管理 Agent 上下文（记忆、资源、技能）——层次化、可浏览、可调试。
  - **L0/L1/L2 三层加载**：L0（始终加载的核心上下文）、L1（按需加载的目录级上下文）、L2（从 VikingDB 向量搜索深度检索）——渐进式披露，而非一次性注入。
  - **技能自演化**：从积累的记忆和交互模式中自动提炼可复用技能。
  - **记忆轨迹可视化**：目录递归检索让记忆从黑盒变白盒。
- **后端**：VikingDB（字节跳动向量数据库），需要 Volcengine 账号
- **链接：** [GitHub](https://github.com/volcengine/OpenViking) | [文档](https://www.openviking.ai/docs)
